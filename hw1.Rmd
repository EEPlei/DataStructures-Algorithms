...
is_connected <- function(g, v1, v2){
  ...
  edges <- edge(g)
  for(i in 1:nrow(edges)){
    istart <- edges[i,1] #build array of the start vertices
    iend <- edges[i,2] #build array of the end vertices 
    if(istart == v1){ #if the start is v1 and end is v2, return TRUE
      if(iend == v2){
        return(TRUE)
      } else  {
        if (is_connected(g, iend, v2)){ #displays the condition where v1 connects to iend
          return(TRUE)
        }
      }
    }
