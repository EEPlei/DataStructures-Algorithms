...
is_valid <- function(g){
  ...
  for(i in 1:length(g)){
    ...
    if(typeof(g[[i]]) != "list")
      return(FALSE)
    #below is to check if every element only contains edges and weights
    if(all(names(g[[i]]) != c("edges","weights")))
        return(FALSE)
    #below is to check if the types of edges are all integer 
    if(all(typeof(g[[i]]$edges) != "integer"))
      return(FALSE)
    #below is to check if the types of weights are all numeric
    if(all(class(g[[i]]$weights) != "numeric"))
      return(FALSE)
