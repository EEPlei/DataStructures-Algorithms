...
is_isomorphic = function(g1, g2){
  ...
  for(i in 1:length(g1)){
    if(!(TRUE %in% (names(g1[i]) == names(g2)))){
      stop("Name of vertices are not the same")}
      j = which(names(g1[i]) == names(g2))
      if(length(unlist(g1[[i]])) != length(unlist(g2[[j]]))){
          stop("Vertices do not have the same number of edges")
        } 
      for(k in 1:(length(unlist(g1[[i]])))/2){
        c = g1[[i]]$edges[k]
        d = which(names(g1[c]) == names(g2))
        j = which(names(g1[i]) == names(g2))
        if(FALSE == (d %in% g2[[j]]$edges)){
...
is_connected <- function(g, v1, v2) {
    edges <- edge(g)
    return(is_connected_helper(edges, v1, v2, c())) # call up is_connected_helper, to use the "seen" array : this is to prevent infinite loops
}

is_connected_helper <- function(edges, v1, v2, seen) { #"seen" array denotes the vertices that I've already passed 
    ...
    for (i in 1:nrow(edges)) {
        ...
        if (istart == v1 & (iend == v2 | is_connected_helper(edges, iend, v2, seen))) { #if edge start is v1 and edge end is v2 or if edge start is v1 and edge end is some other vertex other than v2 
          return(TRUE)
        }
    }
    return(FALSE)
